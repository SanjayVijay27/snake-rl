...
class Linear_QNet(nn.Module):
    ...
    def save(self, file_name="model.pth"):
        model_folder_path = (r"C:\Users\sanja\Desktop\AI Camp\Snake RL\model_outputs")
...
class QTrainer:
    def __init__(self, model, lr, gamma):
        ...
        self.lr = lr
        self.gamma = gamma
        self.optimer = optim.Adam(model.parameters(), self.lr)
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        for index in range(len(done)):
            Q_new = reward[index]
            if not done[index]:
                Q_new = reward[index] + self.gamma * torch.max(self.model(next_state[index])).cuda()
